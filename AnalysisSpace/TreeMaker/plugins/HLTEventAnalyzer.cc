  ...
  triggerEventTag_(ps.getParameter<edm::InputTag>("triggerEvent"))
{
  ...
  cout << "HLTEventAnalyzer configuration: " << endl
...
void HLTEventAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (!triggerResultsHandle_.isValid()) {
    cout << "HLTEventAnalyzer::analyze: Error in getting TriggerResults product from Event!" << endl;
    return;
  }
  iEvent.getByToken(_srcTriggerEventToken, triggerEventHandle_);
  if (!triggerEventHandle_.isValid()) {
    cout << "HLTEventAnalyzer::analyze: Error in getting TriggerEvent product from Event!" << endl;
    return;
  }
  // sanity check
