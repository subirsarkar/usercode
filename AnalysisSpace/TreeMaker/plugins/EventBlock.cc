...
void EventBlock::analyze(edm::Event const& iEvent, edm::EventSetup const& iSetup) {
  ...
  if (found && primaryVertices.isValid()) {
    ...
    for (auto it = primaryVertices->begin(); it != primaryVertices->end(); ++it) {
      if (!it->isFake() &&
           it->ndof() > vtxMinNDOF_ &&
	   std::abs(it->z()) <= vtxMaxAbsZ_ &&
	   std::abs(it->position().rho()) <= vtxMaxd0_)
      {
  ...
  if (!iEvent.isRealData()) {
    ...
    if (found && puInfo.isValid()) {
      for (auto it = puInfo->begin(); it != puInfo->end(); ++it) {
	ev.bunchCrossing.push_back(it->getBunchCrossing()); // in-time and out-of-time indices
	bunchCrossing_->push_back(it->getBunchCrossing());

	ev.nPU.push_back(it->getPU_NumInteractions());
	nPU_->push_back(it->getPU_NumInteractions());

	ev.trueNInt.push_back(it->getTrueNumInteractions());
	trueNInt_->push_back(it->getTrueNumInteractions());
      }
    }
