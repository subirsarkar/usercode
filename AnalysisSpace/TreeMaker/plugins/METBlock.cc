                       ...
                       const edm::EDGetTokenT<pat::METCollection>& token)
{
  ...
  if (found && metColl.isValid()) {
    ...
    for (auto it = metColl->begin(); it != metColl->end(); ++it) {
      ...
      mobj.metphi = it->phi();
      mobj.sumet = it->sumEt();
      mobj.metuncorr = it->uncorrectedPt(pat::MET::uncorrALL);
      mobj.metphiuncorr = it->uncorrectedPhi(pat::MET::uncorrALL);
      mobj.sumetuncorr = it->sumEt() - it->corSumEt(pat::MET::uncorrALL);
