...
process.isolatedPatTaus = cms.EDFilter('PATTauSelector',
  ...
  cut = cms.string('pt > 20 & abs(eta) < 2.3 & tauID("decayModeFinding") > 0.5 & (tauID("againstMuonLoose") > 0.5) & (tauID("againstElectronLoose") > 0.5 || tauID("againstElectronMVA") > 0.5 || 
  tauID("againstElectronVLooseMVA2") > 0.5) & (tauID("byLooseCombinedIsolationDeltaBetaCorr") > 0.5 || tauID("byLooseIsolationMVA") > 0.5)'),
  filter = cms.bool(False))
